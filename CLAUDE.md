...
- ✅ S3 Full site + SEO — **DONE**
  ...
  - ✅ Polish pass (2026-06-21) — 6 user-reported issues, all confirmed and fixed:
    1. Dr. Nelson photo had visible gradient "bars" on both sides in `DoctorSection.tsx`/About hero — was `object-contain` in a box wider than the (near-square) photo; switched to `object-cover`.
    2. Footer's `serviceLinks` listed 3 fabricated services (Veneers, Emergency Care, Pediatric Dentistry) that don't exist on `/services` — clicking them landed at page-top since the anchor had no matching element. Fixed to the real 8 service IDs.
    3. Only Home/Services hero had the `animate-orb-float` gradient treatment; About/Blog (index+article)/Contact/First Visit/Insurance heroes were flat. Applied the same orb pattern to all 6.
    4. About's "Our space" gallery only used 6 of the 12 real office photos available on medismilegroup.com (one is a near-duplicate, so 11 are used) — expanded it, and replaced the "Care team" section's photo (a masked-woman portrait with zero connection to bilingual messaging) with the lounge photo + an "EN / PT / ES" badge.
    5. Home's "Locations" section had a literal `<div>` placeholder labeled "Google Maps embed in Sprint 3." A real embed doesn't need an API key — `https://maps.google.com/maps?q=<address>&output=embed` works with zero credentials. Added it for both locations — but this also required updating `next.config.mjs`'s CSP, which had `frame-src 'none'` (added by the earlier Cowork session) and would have silently blocked the iframe; changed to allow `https://www.google.com https://maps.google.com`.
    6. Blog posts never had cover images (gradient placeholder only) — added a `coverImage` field to `BlogPost` in `blog-posts.ts`, reusing 5 existing service photos + 2 About office photos (no new sourcing needed), rendered on both the index cards and the article template.
  - ✅ Orb intensity fix (2026-06-21, same session) — after item 3 above, user looked at the **Services** hero (which already had orbs, untouched this sprint) and said it still looked like a static blue background. Correct call: `animate-orb-float` only translated 30px vertically, and every orb was rendered at 5–15% opacity through `blur-3xl` — present but not perceptible. Fixed at the source: the keyframe (`globals.css`) now does a diagonal translate + scale pulse across 3 keyframe steps instead of a flat 2-state vertical bob, and every orb instance site-wide (`HeroSection.tsx` + the 7 generic hero sections) had its opacity roughly tripled (`/10`→`/30`, `/5`→`/25`, `/15`→`/35`, `/8`→`/30`) and blur softened from `blur-3xl` to `blur-2xl` so the shape actually reads. Verified with two Playwright screenshots a few seconds apart on a production build — the glow is now clearly visible (previously indistinguishable from the flat gradient in the same test).
...
- S6 QA & go-live
- S7 i18n (EN/PT/ES) — **planned, not started.** 2026-06-21: confirmed the site must serve 3 language communities (English, Brazilian Portuguese, and Spanish/Latino — Florida). This reverses an earlier decision to keep i18n PT-only. Scope: `next-intl`, `/[locale]/(site)/...` routing, full content translation across every page, language switcher, hreflang tags. Sofia (chatbot) already auto-detects/replies in all 3 languages today — that part doesn't need this sprint. Do not start this unprompted; treat as its own dedicated sprint when opened.
...
- `hero/`: clinic-office.jpg · dentista-hero.jpg · sorriso-bg.png — all three are verified byte-identical to images on medismilegroup.com (this project's "real photos" were sourced from there originally). `dentista-hero.jpg` is currently unused (was in the About "care team" slot until 2026-06-21, replaced for being disconnected from the bilingual messaging there — see roadmap).
...
- `about/`: lounge.jpg · lounge-wide.jpg · reception.jpg · treatment-room.jpg · hallway.jpg · hallway-signage.jpg · sterilization-hallway.jpg · consultation-room.jpg · coffee-corner.jpg · entrance.jpg · exterior.jpg — 11 of the 12 real interior/exterior photos of the actual office, sourced from a CSS-background-image gallery on medismilegroup.com (not visible to plain HTML scraping — found via headless-browser DOM inspection; one near-duplicate consultation-room angle was skipped). 10 are in the About page's "Our space" grid; `lounge-wide.jpg` is reused for the "Care team" section and `reception.jpg`/`lounge.jpg` are also reused as blog cover images (see `blog-posts.ts`) — reusing a photo across sections is fine, don't duplicate the file.
